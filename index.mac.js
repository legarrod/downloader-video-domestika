...
// ------------------------------
// NUEVO: Lógica “Solo Subtítulos”
// ------------------------------
async function downloadSubtitlesOnly(courseUrl, subtitle_lang, auth, courseTitle) {
    if (!subtitle_lang) {
      console.error('❌ Debes indicar un idioma de subtítulos.');
      process.exit(1);
    }
    ...
    // Paso 1: carga el curso y extrae los href de cada unidad
    await page.goto(courseUrl, { waitUntil: 'networkidle2' });
    const unitHrefs = await page.$$eval(
      'h4.h2.unit-item__title a',
      els => els.map(el => el.getAttribute('href'))
    );
    console.log(`🔍 Unidades encontradas: ${unitHrefs.length}`);
  
    let total = 0, downloaded = 0;
  
    // Paso 2: por cada unidad, carga sus videos
    for (let i = 0; i < unitHrefs.length; i++) {
      const href = unitHrefs[i].startsWith('http')
        ? unitHrefs[i]
        : `https://www.domestika.org${unitHrefs[i]}`;
  
      await page.goto(href, { waitUntil: 'networkidle2' });
      
      // Aquí sí declaramos data y videoData con let/const
      ...
      for (let j = 0; j < videos.length; j++) {
        const el = videos[j];
        // ¡Aquí sí existe videoData y vData!
        const vData = {
        ...
        const fileName = `${courseTitle} - U${i+1} - ${j+1}_${vData.title}`;
        ...
        try {
          console.log(`🎧 Descargando subtítulos para: ${fileName}`);
          await exec(
            `./N_m3u8DL-RE --auto-subtitle-fix --sub-format SRT ` +
            `--select-subtitle lang="${subtitle_lang}":for=all ` +
            `"${vData.playbackURL}" ` +
            `--save-dir "${finalDir}" ` +
            `--save-name "${fileName}" ` +
            `--tmp-dir ".tmp" --log-level OFF`,
            { maxBuffer: 1024 * 1024 * 100 }
          );
          console.log(`✔ Guardado: ${path.join(finalDir, fileName + '.' + subtitle_lang + '.srt')}`);
          downloaded++;
        } catch (err) {
          console.error(`❌ Falló "${fileName}": ${err.message}`);
        }
      }
    }
  
    console.log(`\n🎉 Subtítulos descargados: ${downloaded}/${total}`);
    await browser.close();
  }
...
async function main() {
    try {
        ...
        const answers = await inquirer.prompt([
            ...
            },
            {
                ...
                choices: [
                    ...
                    { name: 'Solo traducciones', value: 'translations' }
                ]
            }
        ]);
        ...
        // Si sólo quiere subtítulos:
        if (answers.downloadOption === 'translations') {
            for (const info of courseUrls) {
            console.log(`\n📥 Subtítulos para: ${info.courseTitle}`);
            await downloadSubtitlesOnly(info.url, 'es', auth, info.courseTitle);
            }
            process.exit(0);
        }
